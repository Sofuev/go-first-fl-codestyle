...
func attack(charName, charClass string) string {
	if charClass == "warrior" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, randInterval(8, 11))
	}

	if charClass == "mage" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, randInterval(10, 15))
	}

	if charClass == "healer" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, randInterval(2, 4))
	}
...
func defence(char_name, char_class string) string {
	if char_class == "warrior" {
		return fmt.Sprintf("%s блокировал %d урона.", char_name, randInterval(15, 20))
	}

	if char_class == "mage" {
		return fmt.Sprintf("%s блокировал %d урона.", char_name, randInterval(8, 12))
	}

	if char_class == "healer" {
		return fmt.Sprintf("%s блокировал %d урона.", char_name, randInterval(12, 15))
	}
	return "неизвестный класс персонажа"
}
